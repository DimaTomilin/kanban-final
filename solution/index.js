...
function generationTasklist(){
    ...
    for(const [taskType, taskList] of Object.entries(allTasks)){
        const taskListElement = document.querySelector(`[data-type=${taskType}]`).querySelector("ul");
...
function checkedKeyDown(event){
    const key = parseInt(event.key)
            if(key > 3 || event.altKey === false || !event.code.includes("Digit")){ //Check that key is exist
                throw Error ;
            } else {return key}
...
function deletingImportantTask(task){
    ...
    if(importantTasks.includes(task)){ //Deleting from importantTasksArray deleted tasks
...
function getTasks() {
    ...
    } else { return getTasksFromStorage()}
}

function getImportantTasks(){
    if(!getImportantTasksFromStorage()){
        updateImportantTasksInLocalStorage([]);
    } else { return getImportantTasksFromStorage()}
}
...
//     localStorage.important = JSON.stringify(importantTasks)
//     localStorage.tasks = JSON.stringify(localStorageObject);
// } else {
//     importantTasks = JSON.parse(localStorage.getItem("important"));
//     localStorageObject = JSON.parse(localStorage.getItem("tasks"));
//     generationTasklist();
//     generationImportantTasks(importantTasks)
// }
